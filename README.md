...
## 🌳프로젝트 의의
- 리액트로 진행한 첫 프로젝트로, 상태 관리와 UI를 기능에 따라 컴포넌트 단위로 분리하는 연습을 통해 리액트의 개념과 컨셉을 이해할 수 있었다.
- 전체 UI에 영향을 주는 ‘다크모드’와 ‘전체 삭제’ 기능을 구현하기 위하여 useContext를 통해 전역으로 상태를 관리하는 방법에 대해 이해하고 직접 구현해볼 수 있었다.
- PostCSS를 사용하여 컴포넌트 단위로 스타일을 적용하여 컴포넌트 별로 스타일이 고유하게 관리되는 방식을 적용해볼 수 있었다.
- 차트와 데이터를 연결한 UI를 구현하기 위해 검색을 통해 필요한 라이브러리를 찾아냈고, chart.js 라이브러리를 사용해 볼 수 있었다. chart.js를 리액트와 결합했을 때 실시간으로 반영되어 읽어내는 데이터와 아닌 데이터들을 구분하여 원하는 동작을 이끌어낼 수 있었다.
